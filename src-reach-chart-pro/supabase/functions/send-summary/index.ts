...
async function generateAIInsights(goals: any[], period: string): Promise<string> {
  ...
  if (!LOVABLE_API_KEY) {
    console.log('No LOVABLE_API_KEY, using fallback insights');
...
`;
  ...
  try {
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { 
            role: 'system', 
            content: `You are a supportive productivity coach writing a ${period} email summary. Write 2-3 short, encouraging paragraphs about the user's progress. Include specific observations about their goals and 1-2 actionable tips. Keep it warm and motivating. Do not use markdown formatting - write plain text suitable for an HTML email.` 
          },
          { role: 'user', content: goalsData }
        ],
      }),
    });
    ...
    return data.choices?.[0]?.message?.content || generateFallbackInsights(goals, period);
  } catch (error) {
